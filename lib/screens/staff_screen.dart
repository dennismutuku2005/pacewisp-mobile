      ...
      child: Row(
        ...
        children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('ADMIN ACCESS & POLICY CONTROL', style: TextStyle(color: PaceColors.getDimText(isDark), fontSize: 9, fontWeight: FontWeight.w600, letterSpacing: 2)),
          ]),
  ...
  Widget _buildStaffCard(dynamic s, bool isDark) {
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            Text(s['status']?.toString().toUpperCase() ?? 'ACTIVE', style: TextStyle(fontSize: 8, fontWeight: FontWeight.w600, color: s['status'] == 'active' ? PaceColors.emerald : Colors.red)),
          ]),
        ]),
      ),
    );
   ...
   Widget _buildField(String l, TextEditingController c, IconData i, bool isDark, {bool isPass = false, bool enabled = true}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(l, style: TextStyle(fontSize: 8, fontWeight: FontWeight.w600, color: PaceColors.getDimText(isDark), letterSpacing: 1.5)),
      const SizedBox(height: 8),
      TextField(controller: c, enabled: enabled, obscureText: isPass, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: PaceColors.getPrimaryText(isDark)), decoration: InputDecoration(prefixIcon: Icon(i, size: 14, color: Colors.grey), filled: true, fillColor: PaceColors.getSurface(isDark), border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide.none))),
    ]);
  }
